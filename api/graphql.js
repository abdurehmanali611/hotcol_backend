...
const resolvers = {
  ...
  Query: {
    ...
    cashouts: async (_, {HotelName}, context) => {
      ...
      return await prisma.cashouts.findMany({
        where: whereClause,
      });
    }
  },
  Mutation: {
    ...
    CreateCashout: async (_, { Amount, Reason, HotelName }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
    ...
    verifyAdminPassword: async (_, { HotelName, passwordInput }) => {
      ...
      return await bcrypt.compare(passwordInput, admin.Password);
    },
    CreateCredential: async (_, { UserName, Password, Role, HotelName, LogoUrl }) => {
      const existingUser = await prisma.user.findUnique({
        where: { UserName: UserName },
      });
    ...
    UpdateCredential: async (_, { UserName, Password, HotelName, Role }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
      const hashedPassword = await bcrypt.hash(Password, 12);
      const user = await prisma.user.findFirst({
        where: { HotelName: HotelName, Role: Role },
      });
    ...
    CreateItem: async (_, { name, price, category, imageUrl, HotelName, type }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
      return await prisma.item.create({
        data: { name, price, category, type, imageUrl, HotelName },
      });
    },
    OrderCreation: async (_, {
      title, imageUrl, tableNo, waiterName, status, payment, category,
      type, price, HotelName, orderAmount
    }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
      return await prisma.order.create({
        data: {
          title, imageUrl, tableNo, waiterName, orderAmount,
          status, HotelName, payment, category, type, price,
        },
      });
    },
    ...
    UpdateItem: async (_, { id, name, category, price, imageUrl, HotelName, type }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
    ...
    CreateTable: async (_, { tableNo, HotelName, capacity }, context) => {
      ...
      return await prisma.table.create({
        data: { tableNo, HotelName, capacity, price: [], payment: [] },
      });
    },
    ...
    DeleteWaiter: async (_, { id }, context) => {
      if (!context.user) throw new Error("Not Authenticated");
      return await prisma.waiter.delete({ where: { id: id } });
    },
