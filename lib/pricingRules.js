...
/** Fee tiers — café module affects lodging lookup keys; required for cafés but not tier key there. */
...
export function modulesForPricingLookup(modules, businessType) {
  const bt =
    businessType != null ? normalizePricingBusinessType(businessType) : "";
  const lodging = ["Hotel", "Resort", "Pension"].includes(bt);

  return parseModulesJson(modules).filter((m) => {
    const name = String(m).trim();
    if (PRICING_TIER_MODULE_NAMES.has(name)) return true;
    return lodging && name === "Cafe and Restaurant";
  });
}
...
export function calculateSignupPricingHardcoded(businessType, modules) {
  ...
  const lodging = ["Hotel", "Resort", "Pension"].includes(normalized);
  const tierSet = new Set(modulesForPricingLookup(modules, normalized));
  const hasCafe = tierSet.has("Cafe and Restaurant");
  const hasInv = tierSet.has("Inventory");
  const hasFin = tierSet.has("Financial Management");
  const hasCredit = tierSet.has("Credit Management");

  if (normalized === "Cafe and Restaurant") {
...
export function buildModulesKey(modules, businessType) {
  const list = modulesForPricingLookup(modules, businessType)
    .map((m) => String(m).trim())
    .filter(Boolean);
...
export async function findPricingRule(businessType, modules) {
  ...
  const modulesKey = buildModulesKey(modules, bt);
  if (!bt) return null;
