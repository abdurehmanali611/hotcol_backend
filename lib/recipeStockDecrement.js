...
/**
 * Find station on-hand by case-insensitive name.
 * Searches all tenant HotelName keys (TIN + display) so stock credited under
 * either key still matches recipe deduction.
 * If duplicate casing rows exist (Milk + milk), prefer the one with stock.
 */
...
export async function creditStationIngredientStock(
  ...
  if (typeof normalizeStation === "function") {
    ...
    if (before === 0) {
      ...
      if (after > 0) {
        return findStationStockRow(client, scopeKeys, station, name);
      }
    }
  }

  const existing = await findStationStockRow(
    client,
    scopeKeys,
    station,
    name,
  ...
  if (existing) {
    return client.stationIngredientStock.update({
      ...
      data: {
        ...
        // Keep existing casing to avoid duplicate Milk/milk rows under CS collations.
        itemName: String(existing.itemName || name).trim() || name,
      },
    });
  }
 ...
 * Name match is case-insensitive (Milk === milk).
 */
export async function debitStationIngredientStock(
  ...
  if (!existing) {
    try {
      ...
      // Unique clash (e.g. case-insensitive DB collation) — row exists; treat as empty.
    }
    return { applied: 0, shortfall: needed };
  }
 ...
 * Ingredient match is case-insensitive (recipe "Milk" matches station "milk").
 */
