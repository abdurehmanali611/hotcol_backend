...
export function resolveLoginAccess(user, subscription, periodStatus) {
  ...
  const renewalKind = subscriptionRenewalPaymentKind(businessType);
  const isYearly = renewalKind === "yearly";

  if (periodStatus === "on_hold") {
    return {
      accessMode: "denied",
      message:
        "This property is on billing hold. Login is disabled until Apex releases the hold. For help, contact Apex on WhatsApp: +251935000642 or +251930272975.",
    };
  }

  if (periodStatus === "exempt" || periodStatus === "trial") {
    return { accessMode: "full", paymentKind: null };
  }

  if (periodStatus === "trial_ending") {
    return { accessMode: "full", paymentKind: null };
  }

  if (periodStatus === "trial_expired") {
    if (isAdminManager) {
    ...
    return {
      accessMode: "denied",
      message:
        "Your free trial has ended. Login is disabled until the Admin or Manager completes the setup payment. For help, contact Apex on WhatsApp: +251935000642 or +251930272975.",
    };
  }

  const needsRenewalPortal =
  ...
  if (needsRenewalPortal) {
    if (isAdminManager) {
      return { accessMode: "payment_portal", paymentKind: renewalKind };
    }
    return {
      accessMode: "denied",
      message:
        periodStatus === "expired"
          ? isYearly
            ? "Login is disabled — the 10-day grace period after your yearly subscription ended has passed. Pay Apex and contact support to restore access."
          ...
          : isYearly
            ? "Yearly subscription renewal is in progress. Staff terminals are paused until Admin or Manager completes payment."
