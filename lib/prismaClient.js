...
function resolveConnectionLimit() {
  ...
  // Keep serverless pools small so many warm lambdas don't exhaust Aiven.
...
export function createPrismaClient() {
  ...
  const adapter = new PrismaMariaDb({
    ...
    connectTimeout,
    // Must cover a full new TCP+TLS handshake to Aiven (often 2–10s from Vercel).
    acquireTimeout: connectTimeout + 5_000,
    // Seconds — release idle sockets so Aiven slots free across lambdas.
    idleTimeout: 60,
    minimumIdle: 0,
    ssl:
